  ...
  const ChatGPTUI = {
    ...
    showToast(text, options = {}) {
      ...
      if (!el) {
        ...
        el.style.cssText = [
          ...
          error ? 'background: #dc2626' : 'background: #1f2937',
          'color: white',
          'font-size: 14px',
          'padding: 12px 20px',
          'border-radius: 8px',
      ...
      el.textContent = text;
      el.style.background = error ? '#dc2626' : '#1f2937';
      el.style.opacity = '1';
    ...
    addSelectionPopupButton(options) {
      ...
      const observer = new MutationObserver(() => {
        // See canonical chathpt-ui.js@2b51599 — popup switched from
        // position:fixed to translate3d'd absolute in the "Create Note" rollout.
